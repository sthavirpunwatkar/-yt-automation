"""Test script: Upload an existing MP4 to Instagram Reels via Meta Graph API.

Usage:
    python scripts/test_instagram_upload.py --video path/to/video.mp4 --caption "My awesome reel! #Shorts"
"""
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Upload a video to Instagram Reels.")
    ap.add_argument("--video", default="", help="Path to the MP4 file to upload. If empty, uses latest rendered short.")
    ap.add_argument("--caption", default="Test Instagram Reels upload! #FIFA2026 #WorldCup #Reels", help="Reel caption.")
    args = ap.parse_args()
    ...
    try:
        media_id = publish_instagram_reel(video_path, args.caption)
        print(f"\n[OK] Video successfully uploaded to Instagram! Media ID: {media_id}")
